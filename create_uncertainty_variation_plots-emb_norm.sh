...
case $MODE in
        ...
        for var in nominal up down; do
            ...
            for era in 2017; do
                ...
                prefit_postfit_shapes_parallel.py --datacard_pattern "${datacarddir}/${era}/htt_em_2_*/combined.txt.cmb" \
                                                  ...
                                                  --output_name prefit_shapes_emb-norm_${var}.root \
                                                  --parallel 8 |& tee ${defaultdir}/logs/extract_model_independent_shapes-combined-emb-norm_${var}.log
                prefit_postfit_shapes_parallel.py --datacard_pattern "${datacarddir}/${era}/htt_*_3*/combined.txt.cmb" \
                                                  ...
                                                  --output_name prefit_shapes_emb-norm_${var}.root \
                                                  --parallel 8 |& tee -a ${defaultdir}/logs/extract_model_independent_shapes-combined-emb-norm_${var}.log
            ...
            hadd -f ${datacarddir}/combined/cmb/prefit_shapes_emb-norm_${var}.root ${datacarddir}/201?/htt_*/prefit_shapes_emb-norm_${var}.root |& tee -a ${defaultdir}/logs/extract_model_independent_shapes-combined-emb-norm_${var}.log
        done
        ;;
    ...
    do-plots)
        ...
        output=${datacarddir}/plots-variations-emb-norm/
        if [[ ! -d "${output}" ]]
        then
            mkdir ${output}
        fi
        ...
        do
            ./plotting/plot_shapes_mssm_variations.py \
                                           -i ${datacarddir}/combined/cmb/prefit_shapes_emb-norm_nominal.root \
                                           -c ${channels[@]} \
                                           -e 2017 \
                                           $OPTION \
                                           --fake-factor \
                                           ...
                                           -o ${output} \
                                           --up-variation ${datacarddir}/combined/cmb/prefit_shapes_emb-norm_up.root \
                                           --down-variation ${datacarddir}/combined/cmb/prefit_shapes_emb-norm_down.root \
                                           ${cmb_flag} \
                                           --x-range 0,200 \
                                           --name CMS_htt_doublemutrg \
                                           --linear
                                           # --blinded \
        done
        ;;
    ...
    *)
        echo -e "\033[0;31m[ERROR]\033[0m Given mode $MODE not implemented..."
        exit 1
esac
