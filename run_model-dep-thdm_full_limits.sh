...
case "$MODEL" in
    ...
    "THDM_BP1_Type1_mod")
        ...
        scenario_label="#font[42]{Scenario 1 (h,H,A#rightarrow#tau#tau)}"
        sub_analysis="sm-like-light"
        # sm_like_mass="m_h"
        x_title='m_{H} (GeV)'
        # mass_histogram_title="m_{h}"
    ...
    "THDM_PASComp_Type1")
        ...
        scenario_label="#font[42]{Scenario 2 (h,H,A#rightarrow#tau#tau)}"
        sub_analysis="sm-like-light"
        # sm_like_mass="m_h"
        x_title='m_{H} (GeV)'
        # mass_histogram_title="m_{h}"
    ...
    "THDM_HWWLike_Type1")
        ...
        scenario_label="#font[42]{Szenario 3 (h,H,A#rightarrow#tau#tau)}"
        sub_analysis="sm-like-light"
        # sm_like_mass="m_h"
        x_title='m_{H} (GeV)'
        # mass_histogram_title="m_{h}"
...
if [[ $MODE == "initial" ]]; then
    ...
    if [[ $ANALYSISTYPE == "classic" ]]; then
        ...
        morph_parallel.py --output ${defaultdir}/datacards \
            ...
            --sm-like-hists ${sm_like_hists} \
            --sm-gg-fractions ${CMSSW_BASE}/src/CombineHarvester/MSSMvsSMRun2Legacy/data/higgs_pt_reweighting_fullRun2.root \
            --additional-arguments "--auto_rebin=1 --real_data=1 --manual_rebin=1 --enable_bsm_lowmass=1" \
            --eras 2016,2017,2018 \
    ...
    ############
    if [[ $ANALYSISTYPE == "classic" ]]; then
        titleleft='--title-left="Classic categorisation"'
        title="#font[62]{CMS} data 138 fb^{-1} (13 TeV)"
    else
        titleleft=""
        title="#font[62]{CMS} Daten 138 fb^{-1} (13 TeV)"
    fi
    bin_method="--bin-method BinCenterAligned"
    # [[ ${MODEL} =~ "THDM_BP1_Type1" ]] && bin_method=""
    xrange=""
    yrange=""
    if [[ ${MODEL} =~ "BP1" ]]; then
        xrange="--x-range 150,600"
        yrange="--y-range 1,10"
    fi
    for log_arg in "" "--logy"; do
        ${CMSSW_BASE}/src/CombineHarvester/CombineTools/scripts/plotLimitGrid_german.py asymptotic_grid.root \
            --scenario-label="${scenario_label}" \
            --output ${TAG}_${MODEL}${log_arg}_german \
            --title-right="${title}" \
            ${titleleft} \
            --contours="exp-2,exp-1,exp0,exp+1,exp+2,obs" \
            --thdm-validity ${log_arg} \
            ${bin_method} \
            ${xrange} ${yrange} \
            --model_file=${CMSSW_BASE}/src/CombineHarvester/MSSMvsSMRun2Legacy/data/${modelfile} \
            --y-title "tan#kern[0.16667]{#beta}" \
            --debug-output limit_contours_${TAG}_${MODEL}_obs.root \
